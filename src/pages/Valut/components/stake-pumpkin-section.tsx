...
import { DAI, PUMPKIN } from '../../../constants'
import { unwrappedToken } from 'utils/wrappedCurrency'
import CurrencyLogo from 'components/CurrencyLogo'

export default function StakePumpkinSection(
    {
        id, apy, lockTime
    }: {
        id: number, apy: number, lockTime: string
    }) {
    ...
    const stakingInfo = useSingleStakingInfo(PUMPKIN)?.[id]

    const currency = unwrappedToken(PUMPKIN)
            ...
            <section>
                {id == 0 && (
                    <header className="pb-5 border-b border-gray-200 dark:border-zinc-600 sm:flex sm:justify-between sm:items-end">
                        <div>
                            <h3 className="text-lg leading-6 text-gray-900 font-headings dark:text-zinc-50">PUMPKIN</h3>
                            <p className="max-w-3xl mt-2 text-sm text-gray-500 dark:text-zinc-400">
                                When staking PUMPKIN in the security module <span className="font-semibold">you will receive stPUMPKIN</span>{' '}
                                which is a representation of your share of the pool. PUMPKIN in the pool is{' '}
                                <span className="font-semibold">auto-compounding</span>. Your amount of stPUMPKIN{' '}
                                <span className="font-semibold">does not change</span>, but the PUMPKIN value it represents{' '}
                                <span className="font-semibold">will increase</span>. Both PUMPKIN and stPUMPKIN can be used to propose and vote
                                in governance.
                            </p>
                        </div>
                    </header>
                    ...
                    <div className="px-4 py-5 space-y-6 divide-y divide-gray-200 dark:divide-zinc-600 sm:p-6">
                        <div className="md:grid md:grid-flow-col gap-4 sm:grid-cols-[min-content,auto]">
                            ...
                            <div className="mt-3 md:mt-0">
                                <p className="text-sm leading-6 text-gray-500 dark:text-zinc-400 md:mb-1">stPUMPKIN</p>
                                <div>
                            ...
                            <div className="mt-3 md:mt-0">
                                <p className="text-sm leading-6 text-gray-500 dark:text-zinc-400 md:mb-1">PUMPKIN</p>
                                <p className="text-lg font-semibold dark:text-white">
                            ...
                            <div className="mt-3 md:mt-0">
                                ...
                                <p className="text-indigo-600 dark:text-indigo-400">{lockTime}</p>
                            </div>
                                                    ...
                                                    <div className="px-1 py-1">
                                                        <Menu.Item>
                                                            {({ active }: { active: boolean }) => (
                                                                ...
                                                                >
                                                                    {true ? (
                                                                        <Tooltip placement="left" className="mr-2 z-10" label={`You can stake PUMPKIN`}>
                                                            ...
                                                            {({ active }: { active: boolean }) => (
                                                                ...
                                                                >
                                                                    {stakingInfo?.stakedAmount?.greaterThan(JSBI.BigInt(0)) ? (
                                                                        <Tooltip placement="left" className="mr-2 z-10" label={`You can unstake PUMPKIN`}>
                                                                            <div
                                                        ...
                                                        <Menu.Item>
                                                            {({ active }: { active: boolean }) => (
                                                                ...
                                                                >
                                                                    {stakingInfo?.stakedAmount?.greaterThan(JSBI.BigInt(0)) ? (
                                                                        <Tooltip placement="left" className="mr-2 z-10" label={`You can claim PUMPKIN`}>
                                                                            <div
                                                                                className="flex items-center w-full"
